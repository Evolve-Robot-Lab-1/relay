...
try {
  ...
  await waitFor(`/^\\/c\\/G[0-9a-f]+$/i.test(location.pathname) && location.search === ''`, 'The joined invite did not move to its conversation URL.');
  ...
  assert.equal(await evaluate(`document.querySelector('#message-list .message:not(.mine) .who').textContent`), 'Invite Owner');
  ...
  assert.ok((await evaluate(`document.querySelector('#thread-list').textContent`)).includes('Need help saying something? Start here.'));
  ...
  assert.equal(await evaluate(`document.querySelector('label[for="new-message"]').textContent`), 'What do you want the other person to understand?');
  assert.equal(await evaluate(`document.querySelector('#new-message').placeholder`), 'Example: arrange an online meeting tomorrow');
  assert.equal(await evaluate(`document.querySelector('#new-message + .privacy-note').textContent`), 'Only you can see this.');
  ...
  assert.equal(await evaluate(`document.querySelector('#more-button').classList.contains('hidden')`), true, 'Share and destructive actions must stay hidden during first approval.');
  assert.equal(await evaluate(`document.querySelector('#approve-draft-button').textContent`), 'Approve and create invite');
  ...
  assert.equal(await evaluate(`document.querySelector('#draft-status').textContent`), 'Clear, respectful and formal.');
  assert.ok(await evaluate(`Boolean(document.querySelector('#draft-card').compareDocumentPosition(document.querySelector('#message-list')) & Node.DOCUMENT_POSITION_FOLLOWING)`), 'Approval card must appear before the thread.');

  await evaluate(`document.querySelector('[data-tone="friendly"]').click()`);
  await waitFor(`document.querySelector('[data-tone="friendly"]').classList.contains('active') && document.querySelector('#draft-status').textContent.includes('Warm and approachable.')`, 'Friendly tone did not finish.');
  ...
  assert.equal(await evaluate(`document.querySelector('#more-button').classList.contains('hidden')`), false);
  assert.equal(await evaluate(`document.querySelector('#invite-cta').classList.contains('hidden')`), false);
  ...
  assert.equal(await evaluate(`document.querySelector('#message-list .message.mine .original-text')?.textContent`), 'Not shared yet');
  assert.equal(await evaluate(`document.querySelector('[data-more="representative"]').textContent`), 'Relay ON');
  ...
  assert.equal(await evaluate(`document.querySelector('[data-more="preview"]').classList.contains('hidden')`), false);
  await evaluate(`document.querySelector('[data-more="preview"]').click()`);
